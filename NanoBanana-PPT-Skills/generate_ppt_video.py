...
def resolve_ffmpeg_path() -> str:
    """
    Resolve FFmpeg executable path with env override.

    Priority:
    1. FFMPEG_PATH from environment/.env
    2. ffmpeg from PATH
    """
...
def generate_ppt_video_from_images(
    ...
    if video_mode in ("both", "local"):
        ...
        composer = VideoComposer(ffmpeg_path=ffmpeg_path)
...
Workflow:
  ...
  2. Have Codex analyze images and generate prompts:
     Run in Codex: "Analyze images in outputs/xxx/images, generate transition prompts"
...
Notes:
  ...
  - Codex must analyze images to create transition_prompts.json
...
""",
    ...
    parser.add_argument(
        "--prompts-file",
        required=True,
        help="Path to transition prompts JSON file (generated by Codex)",
    )
