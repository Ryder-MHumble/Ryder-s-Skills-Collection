...
def build_props(prompt: str, video_width: int, video_height: int, claude_width: int, typing_speed_ms: int) -> str:
    props = {
        "prompt": prompt,
...
def build_render_args(
    ...
    args = [
        ...
        f"--props={build_props(prompt, video_width, video_height, claude_width, typing_speed_ms)}",
    ]
    if prores_profile:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=f"Typing speed in milliseconds per character (default: {DEFAULT_TYPING_SPEED_MS}).",
    )
    parser.add_argument(
        "--fps",
        type=int,
    ...
    if args.typingSpeedMs <= 0:
        raise RuntimeError("--typingSpeedMs must be a positive integer.")
    if args.fps <= 0:
    ...
    render_args = build_render_args(
        ...
        typing_speed_ms=args.typingSpeedMs,
        fps=args.fps,
        codec=args.codec,
