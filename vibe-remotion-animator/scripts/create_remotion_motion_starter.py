#!/usr/bin/env python3
"""Create a small flexible Remotion motion starter from bundled assets."""
...
def safe_slug(value: str) -> str:
    ...
    return slug or "remotion-motion"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a Remotion motion starter project")
    parser.add_argument(
        "output_dir",
        nargs="?",
        help="Directory to create. Defaults to "${HERMES_OUTPUT:-$HOME/workspace/hermes-output}/vibe-remotion-animations"/<name>",
    )
    parser.add_argument("--name", default="remotion-motion", help="Package/project name")
    parser.add_argument("--format", choices=sorted(FORMATS), default="horizontal")
    parser.add_argument("--duration", type=float, default=24.0, help="Duration in seconds")
    ...
    template = Path(__file__).resolve().parents[1] / "assets" / "remotion-motion-starter"
    ...
    (out / "motion-starter.json").write_text(json.dumps(manifest, indent=2) + "\n")
    print(out)
    return 0
