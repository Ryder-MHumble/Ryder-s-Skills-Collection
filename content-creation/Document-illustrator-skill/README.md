...
**Document Illustrator** 是一个 Codex Skill，它能智能理解文档内容，自动归纳核心要点，并为每个主题生成专业配图。无需依赖特定文档格式，AI 会理解内容并生成符合你选择风格的高质量图片。
...
```bash
cd ~/\.codex/skills/document-illustrator
...
├── LICENSE                   # MIT 许可证
├── SKILL.md                  # Skill 定义文件（供 Codex 使用）
├── .env                      # API 密钥配置（需自行创建）
├── .gitignore                # Git 忽略规则
├── scripts/                  # Python 脚本目录
...
**A**: 可以尝试：
1. 在归纳展示阶段，告诉 codex 你的期望，它会重新归纳
2. 尝试不同的风格
3. 调整配图数量（增加或减少）
4. 提供更详细的文档内容
...
**A**: 目前推荐通过 Codex 逐个处理文档。如果需要批量处理，可以编写自定义脚本循环调用 `generate_single_image.py`。
...
| **支持文档格式** | Markdown, 纯文本, PDF 等（任何 codex 可读的格式） |
| **平均生成时间** | 10-20 秒/张 |
| **Python 版本** | 3.8+ |
