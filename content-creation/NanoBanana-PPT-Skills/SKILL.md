...
description: >
  Generate PPT visuals and optional transition videos from source content.
  Supports style selection, slide planning, image generation, and video composition.
  Use for requests about AI PPT generation, slide image generation, and PPT transition videos.
allowed-tools:
  - Read
  - Write
  - Bash(python:*)
  - Glob
  - AskUserQuestion
argument-hint: "<doc-path-or-content> [style] [pages]"
---

# PPT Generator Pro - Codex Skill

## Runtime Compatibility (Windows + Codex)

- Always resolve `SKILL_DIR` dynamically. Prefer this order:
  1. `~/.codex/skills/ppt-generator-pro`
  2. `~/.codex/skills/ppt-generator`
  3. `~/\.codex/skills/ppt-generator-pro`
  4. `~/\.codex/skills/ppt-generator`
  5. Current working directory if it contains this `SKILL.md`
- Use `python` (not `python3`) for commands to maximize Windows compatibility.
- If FFmpeg is not found in `PATH`, use `FFMPEG_PATH` from `.env`.
