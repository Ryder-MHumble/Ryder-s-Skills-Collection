...
- Resolve `SKILL_DIR` dynamically instead of hardcoding `~/\.codex/skills/youtube-clipper`.
- Use `python` command on Windows (avoid hard dependency on `python3`).
- Prefer `python -m yt_dlp` if `yt-dlp` executable is unavailable.
- Prefer `.env` key `FFMPEG_PATH` over hardcoded ffmpeg paths when present.
