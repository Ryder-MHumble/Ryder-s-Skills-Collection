...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\nNote:")
        print("  此脚本在 Codex Skill 中运行时，codex 会自动处理翻译")
    ...
    try:
        ...
        # 创建双语字幕
        # 注意：在实际使用中，codex 会先完成翻译，然后再调用这个函数
        print("\n⚠️  提示：此脚本需要在 Codex Skill 中运行")
        print("   codex 会自动处理翻译逻辑")
        print("   当前仅输出待翻译数据")
